...
date:   2022-11-15
author: Dallin Mason
description: An exploration of the trends and patterns of weather data collected in SLC during the Fall of 2022.
...
In the blog post mentioned before, we scraped data from September 19 to October 14. This data includes temperature, humidity, barometric pressure, wind speed, and dew point. We selected 2 hour intervals over the course of a day from 6am to 10pm. This is a brief explanation of what data was collected. We further decided to expand the original timeframe of the dataset to include more of the transition into fall and so scraped some more data from the 19th of October to the 11th of November.


## General Trends
It is always fascinating to see the trends the weather has over time. So using the data I made some graphs based on certain weather conditions that were recorded.
...
Another thing that is interesting to look at it the trends of the weather over a daily period. I created a table that contains the mean values for all columns of our original dataset but applied to all of our data. Here is what we got:
...
It is cool to compare what general trends we see here. Definitely we would expect temperatures to rise during the day and fall at night but it is also interesting to note humidity levels doing the exact opposite. Furthermore, dew points seem to follow the same trend as humidity but less extreme. 
...
Another question that would be great to answer is whether or not there is a relationship between the humidity and barometric pressure. This would be a fascinating relationship to make some observations on because barometric pressure can affect some sensitive groups and so if we could determine if humidity somehow is related to that then people might make different choices on where the live at what times of the year based on when humidity is highest.
...
This shows a negative relationship between humidity and barometric pressure so as the humidity increases, the barometric pressure drops slightly on average. This is further confirmed by the correlation which is caluclated to be -0.1931. So there is not too strong of evidence that barometric pressure will drop always but that is the general trend that we observed based on our data. 

So overall there does not seem to be too terribly strong of a relationship between humidity and barometric pressure but a slightly negative correlation can be argued based on the data we collected. 
